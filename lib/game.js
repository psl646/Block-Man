    ...
    row.forEach(function(cell, cellIdx){
      ...
      switch (cell) {
        case CellConstants.BLOCKMAN:
          img.src = ImageConstants[that.blockMan.getDirection()];
          break;
        case CellConstants.WALL:
          if (rowIdx === 0) {
            img.src = ImageConstants["CAVE_CEILING"];
          }
          else if (cellIdx === 0) {
            img.src = ImageConstants["CAVE_EAST_WALL"];
          }
          else if (cellIdx === row.length - 1){
            img.src = ImageConstants["CAVE_WEST_WALL"];
          }
          else {
            img.src = ImageConstants["CAVE_FLOOR"];
          }
          break;
        default:
...
Game.prototype.draw = function (canvasEl) {
  canvasEl.width = this.currentWidth();
  canvasEl.height = this.currentHeight();
